...
async function sendReservationEmail(req, res) {
  ...
  try {
    ...
    // Calcular personas que no participan en electroshock
      ...
      html: `
        ...
        <body>
          ...
          <div class="content">
            ...
            <p>Si tienes alguna duda, no dudes en contactarnos:<br>
            📞 Teléfono: [tu teléfono]<br>
            ✉️ Email: info@tigerlasertag.com</p>
          </div>
          <div class="footer">
    ...
    // También enviar un email de notificación al administrador
    await resend.emails.send({
      from: "Tiger Laser Tag <onboarding@resend.dev>",
      to: "pl836261@gmail.com",
      ...
      html: `
        ...
        <p><strong>Total jugadores:</strong> ${people}</p>
        <p><strong>Electroshock:</strong> ${personas_electroshock}</p>
        <p><strong>Total:</strong> €${total_price}</p>
        <p><strong>Menores de 15:</strong> ${menor_edad ? 'Sí' : 'No'}</p>
        <hr>
        <p><a href="${process.env.NEXT_PUBLIC_APP_URL}/admin/reservas?code=${reservation_code}">Ver reserva en el panel</a></p>
      `
    });
